...
mini.extend(mini.Tree, mini.Control, {
	...
	_OnDrawNode: function (node) {
		...
		// 接近于html的结点
		var e = {
			isLeaf: this.isLeaf(node), // 是否叶子
			node: node, // 节点对象
			nodeHtml: nodeHtml, // 节点html
			nodeCls: '',
			nodeStyle: "",
			showCheckBox: showCheckBox, // 是否显示checkbox
			iconCls: this.getNodeIcon(node), // 图标样式
			showTreeIcon: this.showTreeIcon // 是否显示图标
		};
		...
		if (e.nodeHtml === null || e.nodeHtml === undefined || e.nodeHtml === "")
			e.nodeHtml = "&nbsp;";
		return e;
	},
	/**
	...
	_createNodeTitle: function (node, useEdit, sb) {
		...
		if (e.showCheckBox) {
			...
			sb[sb.length] = '<input type="checkbox" id="' + ckid + '" class="' + this._checkBoxCls + '" hidefocus ' + (checked ? "checked" : "") + '/>';
		}
	...
	cascadeChild: function (node, fn, scope) {
		...
		if (nodes) {
			...
			for (var i = 0, l = nodes.length; i < l; i++) {
				var c = nodes[i];
				if (fn.call(scope || this, c, i, node) === false)
					return;
	...
	eachChild: function (node, fn, scope) {
		...
		if (nodes) {
			...
			for (var i = 0, l = list.length; i < l; i++) {
				var o = list[i];
				if (fn.call(scope || this, o, i, node) === false)
					break;
			}
		}
	},
	...
	addNode: function (node, index, parentNode) {
		...
		if (!nodes)
			nodes = parentNode[this.nodesField] = [];
	...
	moveNodes: function (nodes, targetNode, action) {
		...
		if (!nodes || nodes.length == 0 || !targetNode || !action)
			return;
		this.beginUpdate();
		var sf = this;
		for (var i = 0, l = nodes.length; i < l; i++) {
	...
	moveNode: function (node, targetNode, action) {
		...
		if (!node || !targetNode || !action)
			return false;

		if (this.isAncestor(node, targetNode))
			return false;
	...
	_getNodeByEvent: function (e) {
		...
		if (mini.hasClass(e.target, this._nodesCls))
			return null;
	...
	addNodeCls: function (node, cls) {
		var nodeEl = this._getNodeEl(node);
		if (nodeEl)
			mini.addClass(nodeEl, cls);
	},
	removeNodeCls: function (node, cls) {
		var nodeEl = this._getNodeEl(node);
		if (nodeEl)
			mini.removeClass(nodeEl, cls);
	},
	getNodeBox: function (node) {
		var el = this._getNodeEl(node);

		if (el)
			return mini.getBox(el.firstChild);
	},
	...
	_doCheckState: function (node) {

		var ans = this.getAncestors(node);
		ans.push(node);
		for (var i = 0, l = ans.length; i < l; i++) {
			var pnode = ans[i];
			delete pnode._indeterminate;
			var hasCheck = this.hasCheckedChildNode(pnode);

			var checkEl = this._getCheckBoxEl(pnode);
			if (checkEl) {
				checkEl.indeterminate = false;
				if (this.isCheckedNode(pnode)) {
					checkEl.indeterminate = false;
					checkEl.checked = true;
				} else {
					checkEl.indeterminate = hasCheck;
	...
	__OnDblClick: function (e) {
		if (!this.enabled)
			return;
		if (mini.findParent(e.target, this._checkBoxCls))
			return;
	...
	__OnClick: function (e) {
		...
		var node = this._getNodeByEvent(e);
		if (node && node.enabled !== false) {
			if (mini.findParent(e.target, this._eciconCls) && this.isLeaf(node) == false) {
				...
				if (ev.checkRecursive) {

					this.cascadeChild(node, function (cnode) {
						if (checked) {
							this.uncheckNode(cnode);
						} else {
							this.checkNode(cnode);
						}
					}, this);
					var ans = this.getAncestors(node);
					ans.reverse();
					for (var i = 0, l = ans.length; i < l; i++) {
						...
						if (ck)
							...
							this.uncheckNode(pnode);
					}
				}

				if (this.autoCheckParent) {
					this.doAutoCheckParent(node);
	...
	isAllowDrag: function (node) {
		if (!this.allowDrag)
			return false;
		if (node.allowDrag === false)
			return false;
		var e = this._OnDragStart(node);
		return !e.cancel;
	},
	...
	getAttrs: function (el) {
		...
		"enableHotTrack", "showFolderCheckBox", "resultAsTree","autoEscape",
...
mini._TreeDragDrop.prototype = {
	...
	__OnTreeNodeMouseDown: function (e) {
		var node = e.node;
		if (e.htmlEvent.button == mini.MouseButton.Right)
			return;
		var tree = this.owner;
		if (tree.isReadOnly() || tree.isAllowDrag(e.node) == false)
			return;

		if (tree.isEditingNode(node))
			return;
	...
	_OnDragStart: function (drag) {
		...
		this.feedbackEl.innerHTML = tree._getDragText(this.dragData);
	...
	_OnDragMove: function (drag) {
		...
		if (dropTree) {
			...
			if (targetNode && dropTree.allowDrop == true) {
				...
				if (!dropTree.isLeaf(targetNode) && !targetNode[dropTree.nodesField]) {
	...
	getFeedback: function (dropNode, y, way, tree) {
		...
		if (this.dragData.indexOf(dropNode) != -1)
			return "no";
		var IsLeaf = false;
		if (way == 3) {
